...
onMounted(() => {
  fetchHosts()
  refreshInterval.value = setInterval(fetchHosts, 10000)
  timerInterval.value = setInterval(() => { updatedSecondsAgo.value++ }, 1000)
})
          ...
          <tbody class="divide-y divide-gray-700/30">
            ...
            <tr v-else v-for="host in hosts" :key="host.id" class="group hover:bg-gray-700/20 transition-colors">
              ...
              <td class="px-6 py-4 text-right">
                <button @click="confirmDelete(host.id)" class="text-red-400 hover:text-red-600">Eliminar</button>
              </td>
            </tr>
